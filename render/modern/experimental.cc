...
static std::unique_ptr<gpu::StaticBuffer> s_vbo;
static std::unique_ptr<gpu::StaticBuffer> s_ibo;
...
void experimental::init_late(void)
{
    ...
    s_vbo = std::make_unique<gpu::StaticBuffer>(sizeof(Vertex) * vertices.size(), SDL_GPU_BUFFERUSAGE_VERTEX);
    s_vbo->upload<Vertex>(vertices);

    s_ibo = std::make_unique<gpu::StaticBuffer>(sizeof(Uint32) * indices.size(), SDL_GPU_BUFFERUSAGE_INDEX);
