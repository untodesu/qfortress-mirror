#ifndef RENDER_MODERN_GPU_STATIC_BUFFER_HH
#define RENDER_MODERN_GPU_STATIC_BUFFER_HH
#pragma once
...
template<typename T>
void gpu::StaticBuffer::upload(std::span<const T> data, std::ptrdiff_t byte_offset)
{
    upload(std::as_bytes(data), byte_offset);
}

constexpr std::size_t gpu::StaticBuffer::capacity(void) const noexcept
{
    return m_capacity;
}

constexpr SDL_GPUBufferUsageFlags gpu::StaticBuffer::usage(void) const noexcept
{
    return m_usage;
}

constexpr const SDL_GPUBuffer* gpu::StaticBuffer::handle(void) const noexcept
{
    return m_gpu_handle;
}

constexpr SDL_GPUBuffer* gpu::StaticBuffer::handle(void) noexcept
{
    return m_gpu_handle;
}
