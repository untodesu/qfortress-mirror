...
{
    qf::throw_if_not<std::runtime_error>(SDL_Init(SDL_INIT_VIDEO), "SDL_Init for video subsystem failed: {}", SDL_GetError());

    globals::window = SDL_CreateWindow("client", 640, 480, SDL_WINDOW_RESIZABLE | render::window_flags());
    qf::throw_if_not<std::runtime_error>(globals::window, "SDL_CreateWindow failed: {}", SDL_GetError());
